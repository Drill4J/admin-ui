...
import { Menu, Icons, Status } from '@drill4j/ui-kit';
...
export const ScopesList = () => {
  ...
  const { pluginId = '', buildVersion = '' } =
    useParams<{ pluginId: string; buildVersion: string }>();
          ...
          columns={[
            ...
            },
            {
              ...
              Cell: ({ row: { original = {} } = {} }: any) => (
                <div
                  tw="text-20 leading-32 my-6 text-monochrome-black"
                  data-test="scopes-list:coverage"
                >
            ...
            },
            {
              ...
              accessor: 'autoTests',
              Cell: ({ row: { original = {} } = {} }: any) => {
                const coverageByTestType = transformObjectsArrayToObject(
                  original?.coverage?.byTestType as TestTypeSummary[],
                  'type',
                );
                return (
                  <div tw="font-bold text-12 leading-20 text-monochrome-black">
                    {coverageByTestType?.AUTO && (
                      <span>
                        {`${percentFormatter(
                          coverageByTestType?.AUTO?.summary?.coverage?.percentage || 0,
                        )}%`}
                      </span>
                    )}
                ...
                return (
                  <div tw="font-bold text-12 leading-20 text-monochrome-black">
                    {coverageByTestType?.MANUAL && (
                      <span>
                        {`${percentFormatter(
                          coverageByTestType?.MANUAL?.summary?.coverage?.percentage || 0,
                        )}%`}
                      </span>
                    )}
                    <div tw="font-regular text-right text-monochrome-default leading-16">
                      ...
                      const menuActions = [
                        ...
                        active && {
                          ...
                          Content: ({ children }: { children: JSX.Element }) => (
                            <Link
                              to={`/full-page/${agentId}/${buildVersion}/${pluginId}/scopes/session-management-pane`}
                            >
                              {children}
                            </Link>
                          ),
                        ...
                        !active && {
                          ...
                          onClick: () =>
                            toggleScope(agentId, pluginId, {
                              onSuccess: () => {
                                showMessage({
                                  type: 'SUCCESS',
                                  text: `Scope has been ${
                                    enabled ? 'ignored' : 'included'
                                  } in build stats.`,
                                });
                              },
                              onError: () => {
        ...
        <Stub
          icon={
            <Icons.Scope
              ...
              width={157}
              height={157}
              data-test="no-scope-stub:test-icon"
            />
          }
