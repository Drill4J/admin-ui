...
import { requiredArray, composeValidators, required, sizeLimit } from 'forms';
  ...
  return (
    <div className="w-full overflow-hidden">
      ...
      <Wizard
          ...
          if (agentId) {
            ...
            if (isMounted.current) {
              if (data.plugins?.length === 1) {
                ...
                push(
                  `/full-page/${agentId}/${buildVersion}/${plugin}/dashboard${
                    plugin === 'test2code' ? '/methods' : ''
                  }`,
                );
              } else {
        ...
        onSuccessMessage={
          agentId ? 'Agent has been registered' : 'Offline agent has been preregistered'
        }
      >
        <Step
          name="General Settings"
          component={
            agent.agentType === 'Node.js' ? JsGeneralRegistrationForm : JavaGeneralRegistrationForm
          }
      ...
      {isCancelModalOpened && (
        <CancelAgentRegistrationModal
          ...
          message={`Are you sure you want to abort ${
            agentId ? 'agent registration' : 'offline agent preregistration'
