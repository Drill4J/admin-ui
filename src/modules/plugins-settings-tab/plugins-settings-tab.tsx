...
import { matchPath, Route, useLocation, Link } from 'react-router-dom';
...
export const PluginsSettingsTab = ({ agent: { buildVersion = '' } }: Props) => {
  ...
  const { params: { type: agentType = '', id = '' } = {} } =
  ...
  return (
    <div tw="flex flex-col h-full w-full">
      ...
      <div tw="flex justify-between pt-6 pb-6 mr-6 ml-6 text-20">
        ...
        >
          <Button tw="flex items-center justify-center gap-x-2 h-full pl-2 pr-2 text-14" secondary>
            <Icons.Add />
            Add plugin
          </Button>
        </Link>
      ...
      <div tw="flex-grow mr-6 ml-6">
        {installedPlugins.length > 0 ? (
          installedPlugins.map(({ id: pluginId, name, description, version }) => (
            <Link
              to={
                agentType === 'agent'
              ...
              key={pluginId}
            >
              <PluginListEntry key={id} description={description} icon={name as keyof typeof Icons}>
                <div className="flex items-center w-full">
                  <div tw="font-semibold text-14 leading-22 link">{name}&nbsp;</div>
                  {version && (
                    <div tw="text-14 leading-22 text-monochrome-default">({version})</div>
                  )}
                </div>
              </PluginListEntry>
            </Link>
          ...
          <Stub
            ...
            message={`There are no plugins installed on this ${
