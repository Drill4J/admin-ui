...
const MethodInfoValue = styled.div(({ sceleton }: { sceleton?: boolean }) => [
  tw`text-monochrome-default text-14 break-all`,
  sceleton && tw`h-4 animate-pulse w-full bg-monochrome-medium-tint rounded`,
]);

const MethodInfoLabel = styled.div(
  tw`min-w-32px text-left text-14 leading-32 font-bold text-monochrome-black`,
);
...
export const CoveredMethodsByTestSidebar = ({ topicCoveredMethodsByTest }: Props) => {
  ...
  const summary =
    useBuildVersion<MethodsCoveredByTestSummary>(
    ...
    <Modal isOpen onToggle={closeModal}>
      <div tw="flex flex-col h-full">
        <header tw="flex gap-2 items-center h-16 pl-6 pr-6 leading-32 border-b border-monochrome-light-tint">
          ...
          <div tw="text-monochrome-default text-16 leading-24" style={{ height: '20px' }}>
        ...
        <section tw="h-20 pt-2 pb-2 pr-6 pl-6 bg-monochrome-light-tint border-b border-monochrome-medium-tint">
          ...
          <div tw="flex items-center w-full gap-4">
            ...
            <MethodInfoValue sceleton={showSceleton} className="text-ellipsis">
